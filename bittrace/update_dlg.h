...
#ifdef LANG_EN
#define CHECKED_NEWER_VERSION_TEXT L"Have checked newer version %s (%s), update to it?"
#define UPDATE_SUCCESSFUL_TEXT L"Update successfully"
#else
#define CHECKED_NEWER_VERSION_TEXT L"检测到更新的版本 %s (%s), 是否进行更新?"
#define UPDATE_SUCCESSFUL_TEXT L"更新成功"
#endif //LANG_EN
...
#define FILE_DOWNLOAD_TIP_FMT _T( "文件 已下载 %d/%d" )
	...
	VOID on_update_failed( update_tip_info *tip_info)
	{
			...
			&& *tip_info->err_msg == L'\0' )
		{
			...
			//L"更新失败" 
			tmp_text = _get_string_by_id( TEXT_UPDATE_UPDATE_FAILED, L"Updating error"); 
...
_return:
	...
	VOID on_update_successful()
	{
		...
		tmp_text = UPDATE_SUCCESSFUL_TEXT;
...
_return:
	...
	LRESULT on_update_starting( update_tip_info *info )
	{
		...
		tmp_text = CHECKED_NEWER_VERSION_TEXT; 
