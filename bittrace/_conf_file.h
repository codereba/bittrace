...
typedef enum _column_type
{
	...
	COLUME_STRING_TYPE, 
	COLUME_TIME_TYPE, 
	COLUMN_TYPE_BEIGN = COLUME_NUMBER_TYPE, 
	COLUMN_TYPE_END = COLUME_TIME_TYPE, 
	MAX_COLUMN_TYPE 
} column_type; 
...
//typedef INT32 ( CALLBACK *action_log_callback )( psys_action_log log, PVOID param ); 
...
INLINE LRESULT copy_rule_input_param( access_rule_desc *dest_rule, access_rule_ptr_desc *input_param )
{
	...
	for( i = 0; ( ULONG )i < param_num; i ++ )
	{
		...
		else
		{
			...
			{
				wcscpy( dest_rule->desc.params[ i ]->common.name, input_param->desc.params[ i ].common.name ); 
			}
			else
			{
...
INLINE LRESULT add_log( sys_action_desc *action )
{
	...
	*socket_log.desc = L'\0'; 
	...
	if( rule_type == SOCKET_RULE_TYPE )
	{
		...
		wcsncpy( socket_log.proc_name, action->desc.socket.app.app.app_name, _MAX_FILE_NAME_LEN ); 
	...
	else
	{
		...
		wcsncpy( name_log.proc_name, action->desc.common.app.app.app_name, _MAX_FILE_NAME_LEN ); 
		time( &name_log.time );
		wcsncpy( name_log.target_desc, action->desc.common.param0.common.name, _MAX_TEXT_LEN ); 
...
INLINE LPCWSTR get_response_desc( action_response_type type )
{
	...
	{
	case ACTION_ALLOW: 
		desc = _get_string_by_id( TEXT_RESPONSE_ALLOW, _T( "����" ) );
		break; 
	case ACTION_BLOCK:
		desc = _get_string_by_id( TEXT_RESPONSE_BLOCK, _T( "��ֹ" ) ); 
		break; 
	case ACTION_LEARN:
		desc = _get_string_by_id( TEXT_RESPONSE_LEARN, _T( "ѯ��" ) ); 
		break; 
	default:
		break; 
	}
