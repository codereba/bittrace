...
#ifndef LANG_EN
#define GIVE_SUGESTION_TITLE_TEXT L"提出建议"
#define OPTION_DLG_TITLE_TEXT L"选项" 
#define UPDATE_DLG_TITLE_TEXT L"更新比特安全"
#define MAIN_BITTRACE_TITLE_TEXT L"比特跟踪"
#define UI_CUSTOM_DLG_TITLE_TEXT L"UI界面设置"
#define INSTALL_CODEREBA_CERTIFICATE_TIP_TEXT L"WINDOWS7 64位系统需要对加载的驱动签名验证,需要安装CODEREBA私有证书至根和可信发布机构(BITTRACE是完全免费的软件,如果你使用起来满意,请支持1元购买一个证书(http://www.simpai.net/support.php)),使BITTRACE正确工作,CODEREBA保证BITTRACE仅只有跟踪和日志分析功能,且除更新和用户控制之外没有BIT的任何通信,所有系统功能行为都会完全输出,继续?"
#else
...
LRESULT WINAPI check_base_files()
{
	...
	do 
	{
		//应该一开始检查DBGHELP等文件是否正确存在，不正确存在时，提示相应的信息。
		...
		if( ret != ERROR_SUCCESS )
		{
			show_msg( NULL, 
#ifdef _WIN64
				L"dbghelp64.dll" L"文件丢失,BITTRACE不能正常工作,可以重新安装,恢复文件."
#else
				L"dbghelp32.dll" L"文件丢失,BITTRACE不能正常工作,可以重新安装,恢复文件."
#endif //_WIN64	
				); 
		...
		if( ret != ERROR_SUCCESS )
		{
			show_msg( NULL, L"%s文件丢失,BITTRACE不能认证签名,可以重新安装,恢复文件.", 
				L"codereba_cert.cer" ); 
...
_return:
	...
	/*******************************************************************
	向上搜索不正确，使用不便：没有自动FOCUS，按ENTER键无效
	*******************************************************************/
...
#define SEVEN_FW_DONATE_URL _T( "http://www.simpai.net/support_en.php" )
				...
				else if( name == _T( "relearn_btn" ) )
				{
					...
					tmp_text = _get_string_by_id( TEXT_RELEARN_TIP_MSG, L"设置重新学习规则成功" ); 
				...
				{ 
					...
					tmp_text = UI_CUSTOM_DLG_TITLE_TEXT; //_get_string_by_id( TEXT_UI_CUSTOM_TITLE, L"UI界面设置" ); 
					dlg.Create( GetHWND(), tmp_text, UI_WNDSTYLE_DIALOG, 0L, 0, 0, 1024, 738 );
					dlg.SetIcon( IDI_MAIN_ICON ); 
					dlg.CenterWindow();
					dlg.ShowModal(); 
...
#define SEACH_ENGIN_URL_FORMAT_STRING _T( "http://cn.bing.com/search?q=%s" ) 
		...
		else if( msg.sType == _T("menu") )
		{
			...
			if( 0 == ctrl_name.Compare( L"io_filter_btn") )
			{
				...
				dlg.Create( GetHWND(), _T("外设列表"), UI_WNDSTYLE_DIALOG, 0L, 0, 0, 1024, 738 );
				dlg.SetIcon( IDI_MAIN_ICON ); 
				dlg.CenterWindow();
				dlg.ShowModal(); 
			}
...
_normal_start:
		...
		/********************************************************
		注意：
		消息循环中，如果接收到WM_QUIT消息，则需要返回WPARAM参数。
		********************************************************/
	}
