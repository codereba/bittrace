...
/******************************************************************************
对事件通信模型进行重新架构:
1.事件本身只包括事件源的标志属性,进程ID,线程ID(或者再加入唯一属性:生成时间等,用来表示唯一性)
2.单独建立事件源分析集,与事件集独立,在分析事件时,通过事件的源的ID来定位事件源。

NATIVE路径与符号路径中的盘符路径的关联方式,由内核来实现:
由文件系统过滤驱动将所有的对应关系报告至TRACELOG,TRACELOG
中建立符号路径映射表,在输出事件日志时,进行路径转换.
...
NTSTATUS WINAPI open_action_log_db( LPCWSTR file_path, PVOID *handle )
{
	...
	do 
	{
		...
		**************************************************************/
		...
		/*********************************************************************************************
			ID是联结到了ROWID，而ROWID本身是自带有索引的，所以这里可能并不需要建立索引。
		*********************************************************************************************/
...
NTSTATUS WINAPI collect_file_touch_param( sqlite3_stmt *stmt, file_touch *action )
{
	...
	do 
	{
		...
		action->access = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //文件访问权限 
		action->alloc_size = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //文件初始长度 
		action->attrib = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //文件属性 
		action->share = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); //文件共享属性 
		action->disposition = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 4 ); //文件打开/创建选项 
		action->options = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 5 ); //文件打开/创建选项 
		action->path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 6 ); //动作完成结果NTSTATUS
		action->path_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 7 ); //文件全路径 
...
NTSTATUS WINAPI _collect_file_touch_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_len )
{
	...
	do 
	{
		...
		action->do_file_touch.access = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //文件访问权限 
		action->do_file_touch.alloc_size = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //文件初始长度 
		action->do_file_touch.attrib = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //文件属性 
		action->do_file_touch.share = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); //文件共享属性 
		action->do_file_touch.disposition = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 4 ); //文件打开/创建选项 
		action->do_file_touch.options = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 5 ); //文件打开/创建选项 
		action->do_file_touch.path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 6 ); //动作完成结果NTSTATUS
		...
		text = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 7 ); //文件全路径 
...
NTSTATUS WINAPI _collect_file_open_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_file_open.access = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //文件访问权限 
		action->do_file_open.alloc_size = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //文件初始长度 
		action->do_file_open.attrib = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //文件属性 
		action->do_file_open.share = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); //文件共享属性 
		action->do_file_open.disposition = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 4 ); //文件打开/创建选项 
		action->do_file_open.options = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 5 ); //文件打开/创建选项 
...
NTSTATUS WINAPI collect_file_modified_param( sqlite3_stmt *stmt, file_modified *action )
{
	...
	do 
	{
		...
		action->path_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX ); //文件全路径 
...
NTSTATUS WINAPI _collect_file_modified_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		text = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX ); //文件全路径 
...
NTSTATUS WINAPI collect_file_remove_param( sqlite3_stmt *stmt, file_remove *action )
{
	...
	do 
	{
		...
		action->path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); 
		action->path_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //文件全路径 
...
#endif //_DEBUG
		action->replace_existing = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //是否覆盖已存在文件 
		action->path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); 
...
NTSTATUS WINAPI _collect_file_rename_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_file_rename.replace_existing = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //是否覆盖已存在文件 
...
NTSTATUS WINAPI collect_reg_open_key_param( sqlite3_stmt *stmt, reg_openkey *action )
{
	...
	do 
	{
		...
		action->access = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //注册表打开/创建权限 
		action->path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); 
		action->path_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //注册表键路径 
...
NTSTATUS WINAPI _collect_reg_open_key_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_reg_openkey.access = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //注册表打开/创建权限 
		...
		text = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //注册表键路径 
...
NTSTATUS WINAPI _collect_reg_mk_key_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_reg_mkkey.access = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //注册表打开/创建权限 
...
#endif //_DEBUG
		action->path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); 
		action->path_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //注册表键路径 
...
NTSTATUS WINAPI _collect_reg_rm_key_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		text = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //注册表键路径 
...
#endif //_DEBUG
		action->path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); 
		action->key_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //注册表键路径 
...
NTSTATUS WINAPI collect_proc_exec_param( sqlite3_stmt *stmt, proc_exec *action )
{
	...
	do 
	{
		...
		action->target_pid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //数据长度 
		action->path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); 
		action->path_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //注册表值路径 
...
NTSTATUS WINAPI _collect_proc_exec_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_proc_exec.target_pid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //数据长度 
		...
		text = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //注册表值路径 
...
NTSTATUS WINAPI collect_reg_set_val_param( sqlite3_stmt *stmt, reg_setval *action )
{
	...
	do 
	{
		...
		action->type = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //注册表值类型 
		action->data_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //数据长度 
		...
		action->key_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 4 ); //注册表值路径 
		action->value_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 5 ); //注册表键路径名 
...
NTSTATUS WINAPI _collect_reg_set_val_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_reg_setval.type = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //注册表值类型 
		action->do_reg_setval.data_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //数据长度 
		...
		text = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 4 ); //注册表值路径 
		memcpy( action->do_reg_setval.path_name, 
			text, 
			( action->do_reg_setval.path_len + 1 ) << 1 ); 

		text = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 5 ); //注册表键路径名 
...
NTSTATUS WINAPI _collect_reg_get_val_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->type = ( sys_action_type )( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //注册表值类型 
		action->do_reg_getval.info_size = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //数据长度 
										  ...
										  reg_getval *action )
{
	...
	do 
	{
		...
		action->type = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //注册表值类型 
		action->info_size = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //数据长度 
		action->val_name_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); 
		action->path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); 
		action->key_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 4 ); //注册表值路径 
		action->value_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 5 ); //注册表键路径名 
...
NTSTATUS WINAPI collect_proc_write_vm( sqlite3_stmt *stmt, proc_writevm *action )
{
	...
	do 
	{
		...
		action->target_pid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
		action->base = ( POINTER_TYPE )( ULONG_PTR )sqlite3_column_int64( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //写入内存基址 
		action->data_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //试图写入长度 
		action->bytes_written = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); //成功写入长度 
...
NTSTATUS WINAPI _collect_proc_write_vm( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_proc_writevm.target_pid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
		action->do_proc_writevm.base = ( POINTER_TYPE )( ULONG_PTR )sqlite3_column_int64( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //写入内存基址 
		action->do_proc_writevm.data_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //试图写入长度 
		action->do_proc_writevm.bytes_written = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); //成功写入长度 
...
NTSTATUS WINAPI collect_proc_read_vm( sqlite3_stmt *stmt, proc_readvm *action )
{
	...
	do 
	{
		...
		action->base = ( POINTER_TYPE )( ULONG_PTR )sqlite3_column_int64( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //写入内存基址 
		action->data_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //试图写入长度 
		action->bytes_read = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); //成功写入长度 
		action->result = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 4 ); //动作完成结果(NTSTATUS) 
...
NTSTATUS WINAPI _collect_proc_read_vm( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_proc_readvm.target_pid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
		action->do_proc_readvm.base = ( POINTER_TYPE )( ULONG_PTR )sqlite3_column_int64( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //写入内存基址 
		action->do_proc_readvm.data_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //试图写入长度 
		action->do_proc_readvm.bytes_read = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); //成功写入长度 
...
NTSTATUS WINAPI collect_thrd_remote( sqlite3_stmt *stmt, thrd_remote *action )
{
	...
	do 
	{
		...
		action->target_pid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
		action->target_tid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //目标线程ID 
		action->access = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //线程创建权限 
		action->suspended = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); //是否挂起方式创建 
		action->start_vaddr = ( POINTER_TYPE )( ULONG_PTR )sqlite3_column_int64( stmt, ACTION_PARAM_FIELD_INDEX + 4 ); //线程起始地址 
		action->thread_param = ( POINTER_TYPE )( ULONG_PTR )sqlite3_column_int64( stmt, ACTION_PARAM_FIELD_INDEX + 5 ); //线程参数 
		action->path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 6 ); 
...
NTSTATUS WINAPI _collect_thrd_remote( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_thrd_remote.target_pid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
		action->do_thrd_remote.target_tid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //目标线程ID 
		action->do_thrd_remote.access = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //线程创建权限 
		action->do_thrd_remote.suspended = ( BOOLEAN )( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); //是否挂起方式创建 
		action->do_thrd_remote.start_vaddr = ( POINTER_TYPE )( ULONG_PTR )sqlite3_column_int64( stmt, ACTION_PARAM_FIELD_INDEX + 4 ); //线程起始地址 
		action->do_thrd_remote.thread_param = ( POINTER_TYPE )( ULONG_PTR )sqlite3_column_int64( stmt, ACTION_PARAM_FIELD_INDEX + 5 ); //线程参数 
...
NTSTATUS WINAPI _collect_thrd_set_ctxt( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_thrd_setctxt.target_pid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
		action->do_thrd_setctxt.target_tid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //目标线程ID 
...
NTSTATUS WINAPI collect_w32_lib_inject( sqlite3_stmt *stmt, w32_lib_inject *action )
{
	...
	do 
	{
		...
		action->target_pid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
		action->result = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //动作完成结果(NTSTATUS) 
		action->path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); 
		action->path_name_ptr = ( WCHAR* )sqlite3_column_text16( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); 
...
NTSTATUS WINAPI _collect_w32_lib_inject( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_w32_lib_inject.target_pid = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
		action->do_w32_lib_inject.result = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //动作完成结果(NTSTATUS) 
...
NTSTATUS WINAPI collect_net_create_param( sqlite3_stmt *stmt, net_create *action )
{
	...
	do 
	{
		...
		action->local_ip_addr = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
		action->local_port = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //目标进程ID 
		action->protocol = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //目标进程ID 
...
NTSTATUS WINAPI _collect_net_create_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_net_create.local_ip_addr = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
		action->do_net_create.local_port = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //目标进程ID 
		action->do_net_create.protocol = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //目标进程ID 
...
NTSTATUS WINAPI collect_net_connect_param( sqlite3_stmt *stmt, net_connect *action )
{
	...
	do 
	{
		...
		action->protocol = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
...
NTSTATUS WINAPI _collect_net_connect_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_net_connect.protocol = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
...
NTSTATUS WINAPI _collect_net_listen_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_net_listen.protocol = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //目标进程ID 
...
NTSTATUS WINAPI collect_net_http_param( sqlite3_stmt *stmt, net_http *action )
{
	...
	do 
	{
		...
		action->protocol = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //协议 
		action->cmd = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //HTTP命令 
		action->data_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //发送数据长度 
		action->path_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 3 ); 
...
NTSTATUS WINAPI _collect_net_http_param( sqlite3_stmt *stmt, sys_action_record *action, ULONG buf_size )
{
	...
	do 
	{
		...
		action->do_net_http.protocol = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX ); //协议 
		action->do_net_http.cmd = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 1 ); //HTTP命令 
		action->do_net_http.data_len = ( ULONG )sqlite3_column_int( stmt, ACTION_PARAM_FIELD_INDEX + 2 ); //发送数据长度 
...
/****************************************************************************************
由于需要计算出目标位置的日志的真实编号，所以在高频率的进行定位时，会占用很多的性能。
需要考虑加尽可能多的加载较多的日志至缓存中，这样会有更好的性能效果。
****************************************************************************************/
