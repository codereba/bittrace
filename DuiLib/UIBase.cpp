...
//#endif //realloc_safe
        ...
        if( m_ppVoid == NULL ) 
		{
			ASSERT( FALSE && "allocate for the pointer array error, framework of direct ui crushed" ); 
			return false;
		}
    }
...
int CStdString::Format(LPCTSTR pstrFormat, ...)
{
	...
	if( FAILED( hr ) )
	{
		__Trace( L"print the formated string %s error\n", pstrFormat ); 
		//goto _return; 
	}

	Assign( szBuffer ); 
_return:
    return ARRAYSIZE( szBuffer ) - RemainLength;
}
