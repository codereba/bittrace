...
#define DYNAMIC_LOAD_RICH_EDIT 1

#ifndef DYNAMIC_LOAD_RICH_EDIT 
#pragma comment( lib, "Riched20.lib" )
#endif //DYNAMIC_LOAD_RICH_EDIT 
...
BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
{
...
#ifdef DYNAMIC_LOAD_RICH_EDIT
	{
		...
		RichEdModule = LoadLibraryW( L"Riched20.dll" ); 

		if(RichEdModule == NULL)
		{
			RichEdModule = LoadLibraryW( L"Msftedit.dll" ); 

			if( NULL == RichEdModule )
			{
				goto err; 
			}
		}
		...
		{
			CTS = (_CTS)GetProcAddress(RichEdModule, "CreateTextServices");
			if(NULL == CTS)
			{
				goto err; 
			}
...
{
	__Trace( L"%s rech edit pointer is 0x%0.8x\n", __TFUNCTION__, m_re );
    if(fShow)
...
{
    return UI_RICH_EDIT_CLASS_NAME;
}

ULONG CRichEditUI::GetClassId() const
{
	return UI_RICH_EDIT_CLASS_ID;
}

LPVOID CRichEditUI::GetInterface(LPCTSTR pstrName)
{
    if( _tcscmp(pstrName, UI_RICH_EDIT_INTERFACE_NAME) == 0 ) return static_cast<CRichEditUI*>(this);
...
int CRichEditUI::AppendText(LPCTSTR lpstrText, bool bCanUndo)
{
	...
	//text_len = GetTextLength( GTL_DEFAULT ); 
    
	//text_limit = GetLimitText(); 
	//overflow_size = text_len + _tcslen( lpstrText ) - text_limit; 

	//if( overflow_size >= 0 )
	//{
	//	nRet = SetSel( 0, text_len ); 
	//}
	//else
	{
		nRet = SetSel(-1, -1);
