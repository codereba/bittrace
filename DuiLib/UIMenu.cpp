...
bool CMenuUI::Add(CUIControl* pControl)
{
...
bool CMenuUI::AddAt(CUIControl* pControl, int iIndex)
{
	CMenuElementUI* pMenuItem = static_cast<CMenuElementUI*>(pControl->GetInterface(kMenuElementUIInterfaceName));
	if (pMenuItem == NULL)
		return false;
...
int CMenuUI::GetItemIndex(CUIControl* pControl) const
{
	CMenuElementUI* pMenuItem = static_cast<CMenuElementUI*>(pControl->GetInterface(kMenuElementUIInterfaceName));
	if (pMenuItem == NULL)
		return -1;
...
bool CMenuUI::SetItemIndex(CUIControl* pControl, int iIndex)
{
	CMenuElementUI* pMenuItem = static_cast<CMenuElementUI*>(pControl->GetInterface(kMenuElementUIInterfaceName));
	if (pMenuItem == NULL)
		return false;
...
bool CMenuUI::Remove(CUIControl* pControl)
{
	CMenuElementUI* pMenuItem = static_cast<CMenuElementUI*>(pControl->GetInterface(kMenuElementUIInterfaceName));
	if (pMenuItem == NULL)
		return false;
...
SIZE CMenuUI::EstimateSize(SIZE szAvailable)
{
    ...
    for( int it = 0; it < GetCount(); it++ ) {
        CUIControl* pControl = static_cast<CUIControl*>(GetItemAt(it));
...
/////////////////////////////////////////////////////////////////////////////////////
//
class CMenuBuilderCallback: public IUIBuilderCallback
{
...
LRESULT CMenuWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if( uMsg == WM_CREATE ) 
	{
		if( m_pOwner != NULL) 
		{
			...
			for( int i = 0; i < m_pOwner->GetCount(); i++ )
			{
				if(m_pOwner->GetItemAt(i)->GetInterface(kMenuElementUIInterfaceName) != NULL )
				{
					...
					m_pLayout->Add(static_cast<CUIControl*>(m_pOwner->GetItemAt(i)));
				}
			}
			...
			for( int it = 0; it < m_pOwner->GetCount(); it++ ) 
			{
				...
				{
					CUIControl* pControl = static_cast<CUIControl*>(m_pOwner->GetItemAt(it));
					SIZE sz = pControl->EstimateSize(szAvailable);
					cyFixed += sz.cy;
		...
		else 
		{
			...
			CUIControl* pRoot = builder.Create(m_xml, m_sType.GetData(), &menuCallback, &m_pm);
