...
void CComboWnd::Init(CComboUI* pOwner)
{
    ...
    int cyFixed = 0;
    for( int it = 0; it < pOwner->GetItemCount(); it++ ) {
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        for( int i = 0; i < m_pOwner->GetItemCount(); i++ ) {
            m_pLayout->AddItem(static_cast<CControlUI*>(m_pOwner->GetItemAt(i)));
        }
        ...
        if( pControl && _tcscmp(pControl->GetClass(), UI_SCROLL_BAR_CLASS_NAME ) != 0 ) PostMessage(WM_KILLFOCUS);
    }
    else if( uMsg == WM_KEYDOWN ) {
        switch( wParam ) {
        case VK_ESCAPE:
            m_pOwner->SelectListItem(m_iOldSel);
...
void CComboWnd::EnsureVisible(int iIndex)
{
	...
	/***************************************************
	advance this performance of this if sentence, item 
	is often lower than list top ? or bottom is often
	greater than list bottom?
	***************************************************/
...
{
    return UI_COMBO_CLASS_NAME;
}

ULONG CComboUI::GetClassId() const
{
	return UI_COMBO_CLASS_ID;
}

LPVOID CComboUI::GetInterface(LPCTSTR pstrName)
{
	if( _tcscmp(pstrName, UI_COMBO_INTERFACE_NAME) == 0 ) return static_cast<CComboUI*>(this);
...
{
    if( m_iCurSel < 0 ) SelectListItem(0);
}
...
bool CComboUI::SelectListItem( int iIndex )
{
    if( m_pWindow != NULL ) 
	{
		m_pWindow->Close();
	...
	{
		__Trace( _T("select the item that is not in the list, index(%u) too big." ), iIndex ); 
...
bool CComboUI::AddItem(CControlUI* pControl)
{
    IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(UI_LIST_ITEM_INTERFACE_NAME));
    if( pListItem != NULL ) 
    {
    ...
    return CContainerUI::AddItem(pControl);
}

bool CComboUI::AddItemAt(CControlUI* pControl, int iIndex)
{
    if (!CContainerUI::AddItemAt(pControl, iIndex)) return false;
    ...
    for(int i = iIndex + 1; i < GetItemCount(); ++i) {
        CControlUI* p = GetItemAt(i);
...
bool CComboUI::RemoveItem(CControlUI* pControl)
{
    ...
    if( !CContainerUI::RemoveItemAt( iIndex ) )
	{
		return false;
	}
...
bool CComboUI::RemoveItemAt(int iIndex)
{
    if (!CContainerUI::RemoveItemAt(iIndex)) return false;
...
void CComboUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_SCROLLWHEEL )
    {
        ...
        SelectListItem(FindSelectable(m_iCurSel + (bDownward ? 1 : -1), bDownward));
        return;
    }
...
void CComboUI::PaintText(HDC hDC)
{
    ...
    if( m_iCurSel >= 0 ) {
        CControlUI* pControl = static_cast<CControlUI*>(m_items[m_iCurSel]);
        IListItemUI* pElement = static_cast<IListItemUI*>(pControl->GetInterface(UI_LIST_ITEM_INTERFACE_NAME));
        if( pElement != NULL ) {
