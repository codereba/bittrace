...
bool CMarkup::LoadFromMem(BYTE* pByte, DWORD dwSize, int encoding)
{
    ...
    else
    {
        if ( dwSize >= 2 && ( ( pByte[0] == 0xFE && pByte[1] == 0xFF ) || ( pByte[0] == 0xFF && pByte[1] == 0xFE ) ) )
        {
			...
			//this xml text is bigendian,convert it to the little endian.
...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
	...
	else 
	{
		...
		CPaintManagerUI::GetResourceZipResType( res_type, res_name ); 
		if( read_data_from_zip( CPaintManagerUI::GetResourceZip(), res_type, res_name, pstrFilename, &pByte, &dwSize ) != ERROR_SUCCESS )
		{
			return false; 
		}
