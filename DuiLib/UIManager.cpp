...
DuiLib::CAnimationSpooler m_anim;
...
STRINGorID CPaintManagerUI::zip_res_type( (LPCTSTR )NULL ); 
STRINGorID CPaintManagerUI::zip_res_name( (LPCTSTR )NULL );  
...
void CPaintManagerUI::init()
{
	...
	m_mImageHash.Resize( DEF_HASH_TABLE_SIZE ); 
	m_DefaultAttrHash.Resize( DEF_HASH_TABLE_SIZE ); 
	m_mOptionGroup.Resize( DEF_HASH_TABLE_SIZE ); 
}

CClosePaintEffectiveUI::CClosePaintEffectiveUI() : m_bCloseStretchPaint( false ), 
													SavedScreenBmp( NULL ), 
													SavedScreenDC( NULL ) 
{
...
CClosePaintEffectiveUI::~CClosePaintEffectiveUI()
{
	//if( SavedScreenBmp != NULL )
	//{
	//	::DeleteObject( SavedScreenBmp ); 
	//	SavedScreenBmp = NULL; 
	//}

	//m_bCloseStretchPaint = false; 
}

void CClosePaintEffectiveUI::SetCloseStretchPaint( bool Value )
{
	m_bCloseStretchPaint = Value; 

	return; 
}

bool CClosePaintEffectiveUI::GetCloseStretchPaint()
{
	return m_bCloseStretchPaint; 
}


LRESULT CClosePaintEffectiveUI::PaintEffective( HWND hWnd, 
											   RECT &rcPaint, 
											   HDC hDcOffscreen, 
											   HBITMAP hbmpOffscreen )
{
	...
	ASSERT( hbmpOffscreen != NULL ); 
	ASSERT( hDcOffscreen != NULL ); 
	...
	do 
	{
		...
		//if( rcPaint.left == rcPaint.right 
		//	|| rcPaint.top == rcPaint.bottom )
		{
			_ret = GetWindowRect( hWnd, &rcPaint ); 
			if( _ret == FALSE )
			{
				__Trace( _T( "get the rect for the window 0x%0.8x that paint effective error %u\n" ), hWnd, GetLastError() ); 
		...
		if( PaintDC == NULL )
		{
			__Trace( _T( "get the dc for the window 0x%0.8x that paint effective error %u\n" ), hWnd, GetLastError() ); 
			ret = ERROR_ERRORS_ENCOUNTERED; 
			break; 
		}
		...
		//
		//HBRUSH br; 
		//br = CreateSolidBrush( RGB( 233, 233, 233 ) ); 
		//if( br != NULL )
		//{
		//	_ret = FillRect( PaintDC, &rc, br ); 
		//	DeleteObject( br ); 
		//}
		...
		::GetObject( hbmpOffscreen, sizeof(BITMAP), &Bitmap ); 

		old_obj = SelectObject( hDcOffscreen, hbmpOffscreen ); 

		//_ret = ::StretchBlt( PaintDC, 
		//	rc.left, 
		//	rc.top, 
		//	rc.right - rc.left, 
		//	rc.bottom - rc.top, 
		...
		//::GetDIBits( ps.hdc, SavedScreenBmp, 0, 0, NULL, & )
		//::BeginPaint( hWnd, &ps);
		...
		if( _ret == FALSE )
		{
			__Trace( _T( "paint effective on the window 0x%0.8x error %u\n" ), hWnd, GetLastError() ); 
			ret = ERROR_ERRORS_ENCOUNTERED; 
	...
	{
		ASSERT( hDcOffscreen ); 
		SelectObject( hDcOffscreen, old_obj ); 
	}
...
LRESULT CPaintManagerUI::PaintStretchShortEffective( HWND hWnd, 
													RECT &rcPaint )
{
	LRESULT ret = ERROR_SUCCESS; 
	...
	do 
	{
		...
		if( m_hbmpOffscreen == NULL )
		{
			...
			ASSERT( m_hDcOffscreen == NULL ); 
			//ASSERT(m_hbmpOffscreen == NULL );
		...
		ret = PaintEffective( m_hWndPaint, rcPaint, m_hDcOffscreen, m_hbmpOffscreen ); 
	}while( FALSE );
...
void CPaintManagerUI::uninit()
{
	...
	m_mNameHash.Resize( DEF_HASH_TABLE_SIZE ); 
	delete m_pRoot;
...
const VOID CPaintManagerUI:: GetResourceZipResType( STRINGorID& res_type, STRINGorID& res_name )
{
	res_type = zip_res_type; 
	res_name = zip_res_name; 
}
...
void CPaintManagerUI::SetResourceZipResType(STRINGorID res_type, STRINGorID res_name  )
{
	ASSERT( res_name.m_lpstr != NULL && res_type.m_lpstr != NULL ); 

	zip_res_type = res_type; 
	zip_res_name = res_name; 
}
...
void CPaintManagerUI::SetInitSize(INT32 x, INT32 y, int cx, int cy)
{
    m_szInitWindowSize.cx = cx;
    m_szInitWindowSize.cy = cy;
    if( m_pRoot == NULL && m_hWndPaint != NULL ) {
        ::SetWindowPos(m_hWndPaint, NULL, x, y, cx, cy, SWP_NOZORDER /*| SWP_NOMOVE*/ | SWP_NOACTIVATE);
    }
}
    ...
    case WM_PAINT:
        {
			...
			/******************************************************************************************
			使用控制的绘制标志+全控制专用BITMAP图方式绘图优化在下一版本加入
			******************************************************************************************/

			//BOOLEAN NeedRepaint = FALSE; 

			if( m_bOffscreenPaint == true 
				&&
				m_bCloseStretchPaint == TRUE )
			{
				DBG_BP(); 
				...
				PaintEffective( m_hWndPaint, rcPaint, m_hDcOffscreen, m_hbmpOffscreen ); 
				return true; 
			}
            ...
            // We delay this event from the WM_SIZE messages since resizing can be
			...
			/***************************************************************************
			notice the m_hDcOffscreen and m_hbmpOffscreen record the last paint bitmap 
			of the main window.
			***************************************************************************/
			...
			if( m_bOffscreenPaint )
            {
				...
				识别两种不同的绘制原因
				一种是窗体被挡后恢复
				一种是控制的位置区间，或控制的对应数据发生变化。
				需要建立所有控制的绘制位图，此位图之中只可以有所有控制的颜色像素信息。
				以下代码会出现问题。
				***********************************************************************************/
				...
				/***********************************************************************
				draw all contorls traversely.
				***********************************************************************/
				/************************************************************************************
				这种DIRECTUI组织控件的问题在于，一个子控件与其父控件构成的控件链中可能包括很多的层次，
				每个子控件的显示都将是对这个控件链的一次遍历，将会消耗一定的性能(以容器的子控件量有关)。\

				如果有一个子控件高速的刷新窗体显示(INVALIDATE)，那么将会消耗大量的时间。
				所以需要进行一定量的优化，去掉不绘制控件的遍历，控件链搜索算法(当前链表结构)。
				************************************************************************************/
				/***********************************************************************************
				识别两种不同的绘制原因
				一种是窗体被挡后恢复
				...
				***********************************************************************************/
				...
				/***********************************************************************
				paint some thing that's cover on the other pixel.
				***********************************************************************/
    ...
    case WM_MOUSEHOVER:
        {
			...
			//::SendMessage(m_hwndTooltip, TTM_SETDELAYTIME, 0, (LPARAM) &m_ToolTip);
            ::SendMessage(m_hwndTooltip, TTM_SETTOOLINFO, 0, (LPARAM) &m_ToolTip);
            ::SendMessage(m_hwndTooltip, TTM_TRACKACTIVATE, TRUE, (LPARAM) &m_ToolTip);
			::SendMessage( m_hwndTooltip, TTM_SETMAXTIPWIDTH, 0, MAX_TOOLTIP_WIDTH ); //m_ToolTipWidth 
        }
        return true;
    case WM_MOUSELEAVE:
        {
...
BOOL CPaintManagerUI::Invalidate(RECT& rcItem)
{
	BOOL ret; 
	//TRACE( _T( "enter %s" ), __TFUNCTION__ ); 

    ret = ::InvalidateRect( m_hWndPaint, &rcItem, FALSE );
...
bool CPaintManagerUI::InitControls(CControlUI* pControl, CControlUI* pParent /*= NULL*/)
{
	...
	//find control function is to add a hash of its name for it.
...
bool CPaintManagerUI::SetNextTabControl(bool bForward)
{
	...
	{  
        if( bForward ) 
		{
            // Wrap around
            FINDTABINFO info2 = { 0 };
            info2.pFocus = NULL;  //bForward ? NULL : info1.pLast;
        ...
        else 
		{
			...
			//info2.pFocus = info1.pLast;  //bForward ? NULL : info1.pLast;
			//info2.bForward = bForward;
			//
			//pControl = m_pRoot->FindControl(__FindControlFromTab, &info2, UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
...
bool CPaintManagerUI::ChangeFont(HFONT hFont, HFONT NewFont )
{
	return false; 
}

bool CPaintManagerUI::ChangeFont( int index, LPCTSTR pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic )
{
	bool ret = false; 
	TFontInfo* pFontInfo = NULL; 
	...
	do 
	{
		if( index >= m_aCustomFonts.GetSize() )
		{
			break; 
		}
		...
		pOldFontInfo = ( TFontInfo* )m_aCustomFonts.GetAt( index ); 
		ASSERT( pOldFontInfo != NULL ); 
		ASSERT( pOldFontInfo->hFont != NULL ); 

		if( !m_aCustomFonts.SetAt(index, pFontInfo) ) 
		{
			::DeleteObject(hFont);
			delete pFontInfo;
			break; 
...
const TImageInfo* CPaintManagerUI::GetImageEx(LPCTSTR bitmap, LPCTSTR type, DWORD mask, ULONG flags )
{
    TImageInfo* data = static_cast<TImageInfo*>(m_mImageHash.Find(bitmap));
    if( !data ) 
	{
        if( AddImage(bitmap, type, mask, flags ) ) 
		{
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, LPCTSTR type, DWORD mask, ULONG flags )
{
    TImageInfo* data = NULL;
    if( type != NULL ) 
	{
        if( TRUE == isdigit( *bitmap ) ) 
		{
            LPTSTR pstr = NULL;
	...
	{
        data = CRenderEngine::LoadImage( bitmap, NULL, mask, flags );
    }
