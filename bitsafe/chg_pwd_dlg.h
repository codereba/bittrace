	...
	LRESULT on_apply()
	{
		...
		if( ret != ERROR_SUCCESS )
		{
			...
			if( old_pwd != _T( "" ) )
			{
				...
				tmp_text = _get_string_by_id( TEXT_CHANGE_PASSWORD_OLD_INCORRECT_ERROR, _T( "旧密码不正确" ) ); 
		...
		if( new_pwd.GetLength() == 0 )
		{
			...
			tmp_text = _get_string_by_id( TEXT_CHANGE_PASSWORD_NEW_LENGTH_ERROR, _T( "密码将被设置为空,请确认?" ) ); 
...
_return:
		if( ret == ERROR_SUCCESS )
		{
			tmp_text = _get_string_by_id( TEXT_CHANGE_PASSWORD_SUCCESSFUL_TIP, _T( "修改密码成功" ) ); 
