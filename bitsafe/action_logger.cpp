...
#define POP3_PORT_DESC L"[POP3邮件服务]"
#define HTTP_PORT_DESC L"[WEB服务]"
#define FTP_PORT_DESC L"[文件传输服务]"
#define SAMBA_PORT_DESC L"[WINDOWS共享服务]"
#define HTTP_AGANT_PORT_DESC L"[WEB代理服务]"
	switch( port_num )
	{
	case 110:
		desc = POP3_PORT_DESC; 
...
#define EVENT_REQUEST_TIP _T( " 是否允许?" )
	...
	if( flags == LOG_MSG )
	{
		...
		_sntprintf( action_desc, MAX_ACTION_DESC_SIZE, _T( " %s" ), action->resp == ACTION_BLOCK ? _T( "阻止" ) : _T( "允许" ) ); 
...
#define EVENT_REQUEST_TIP _T( " 是否允许?" )
	...
	if( flags == LOG_MSG )
	{
		...
		_sntprintf( action_desc, MAX_ACTION_DESC_SIZE, _T( " %s" ), resp == ACTION_BLOCK ? _T( "阻止" ) : _T( "允许" ) ); 
