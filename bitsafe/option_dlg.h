		...
		case BITSAFE_FUNCTION_OPENED:
			{
				...
				_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_CLOSED_BTN, _T( "已关闭" ) ); 
		...
		case BITSAFE_FUNCTION_DISABLED:
			{
				...
				_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_OPENED_BTN, _T( "已开启" ) ); 
...
_return:
	...
	LRESULT set_option_dlg_lang()
	{
		...
		ASSERT( ctrl != NULL ); 
		ret = _set_ctrl_text( ctrl, TEXT_OPTION_DIALOG_UI_TAB, L"界面" ); 
		ASSERT( ret == ERROR_SUCCESS ); 

		ctrl = static_cast<CButtonUI*>(m_pm.FindControl( _T( "protect_btn" ) ) ); 
		ret = _set_ctrl_text( ctrl, TEXT_OPTION_DIALOG_SECURITY_TAB, L"保护功能" ); 
		ASSERT( ret == ERROR_SUCCESS ); 
...
_return:
	...
	LRESULT ctrl_drv( bitsafe_function_type type, bitsafe_function_state state )
	{
		...
		if( _ret != ERROR_SUCCESS )
		{
			...
			//show_msg( NULL, _T( "保护模块当前没有正常工作,立即进行修复?" ) ); 
			ASSERT( FALSE ); 
			//goto _return; 
		}
		else
...
_return:
	...
	LRESULT set_ui_tab_lang()
	{
		...
		set_ctrl_text( &m_pm, _T( "ui_ok_btn" ), TEXT_COMMON_BUTTON_CONFIRM, _T( "设置" ) ); 
...
_return:
	...
	LRESULT set_shield_tab_lang()
	{
		...
		do 
		{
			...
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_URL_FIREWALL, _T( "URL防火墙" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_arp" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_ARP_FIREWALL, _T( "ARP防火墙" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_netbase" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_BASE_FIREWALL, _T( "网络防火墙基础" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_fw" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_TRFFIC_FIREWALL, _T( "网络防火墙" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_defense" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_DEFENSE_FIREWALL, _T( "主动防御" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_reg" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_REG_SHIELD, _T( "注册表防火墙" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_fs" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_FILE_SHIELD, _T( "文件系统防火墙" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_desc_url" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_URL_FIREWALL_DESC, _T( "保护网站浏览绿色安全" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_desc_arp" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_ARP_FIREWALL_DESC, _T( "保护网络流畅,清除ARP欺骗攻击" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_desc_netbase" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_BASE_FIREWALL_DESC, _T( "保护网络通信的基础" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_desc_fw" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_TRFFIC_FIREWALL_DESC, _T( "保护网络通信透明可信" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_desc_defense" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_DEFENSE_FIREWALL_DESC, _T( "保护系统行为正确安全" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_desc_fs" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_FILE_SHIELD_DESC, _T( "保护数据,文件安全牢固" ) ); 

			ctrl = m_pm.FindControl( _T( "txt_desc_reg" ) ); 
			ASSERT( ctrl != NULL ); 
			( ( CLabelUI* )ctrl )->SetFont( 0 ); 
			_set_ctrl_text( ctrl, TEXT_SECURITY_TAB_REG_SHIELD_DESC, _T( "保护注册表配置安全" ) ); 
