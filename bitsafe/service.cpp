...
//LRESULT get_manage_param_from_conf( ULONG index )
//{
...
//		conf_file_name ); 
//
//	check_delay = atoi( config );
//			sprintf( config, "Process%d\0", i );
//
//GetPrivateProfileString( config, "Restart", "No", config, tmp_buf_size, conf_file_name ); 
//
//if( config[ 0 ] != 'Y' 
//   && config[ 0 ] != 'y' 
//   && config[ 0 ] != '1' )
...
//	continue;
//}
//sprintf( config, "Process%d\0", index ); 
//
//GetPrivateProfileString( config, "CommandLine", "", _cmd_line, tmp_buf_size, conf_file_name ); 
//GetPrivateProfileString( config, "UserInterface", "N", _have_ui, tmp_buf_size, conf_file_name );
//GetPrivateProfileString( config,"WorkingDir", "", working_dir, tmp_buf_size, conf_file_name ); 
//
//have_ui = ( _have_ui[ 0 ] == 'y'
//		   || have_ui[ 0 ] == 'Y' 
//		   || have_ui[ 0 ] == '1' ) ? TRUE : FALSE; 
//
//cmd_line = _cmd_line; 
//cur_dir = working_dir; 
//GetPrivateProfileString( config, "PauseStart", "100", _have_ui, tmp_buf_size, conf_file_name ); 
//start_delay = atoi( _have_ui ); 
///*
...
//						conf_file_name ); 
//
//nPauseEnd = atoi( pPause );*/
//GetPrivateProfileString( "Settings", "ServiceName","secu_mon", SERVICE_NAME, tmp_buf_size, conf_file_name );
//char pCount[tmp_buf_size+1];
//GetPrivateProfileString( "Settings", "ProcCount", "",pCount, tmp_buf_size, conf_file_name );
...
VOID WINAPI service_main( DWORD argc, LPSTR *argv )
{
	...
	//if( status != NO_ERROR ) 
	//{ 
	//	log_trace( ( DBG_MSG_AND_ERROR_OUT, "check service main error code \n" ) ); 
	//}
	//else
	//{
	//	log_trace( ( DBG_MSG_AND_ERROR_OUT, "check service main error code no error\n" ) ); 
	//}
...
VOID WINAPI service_handler( DWORD ctrl_code )
{
	...
	default: 
		//if( ctrl_code >= 128 && ctrl_code < 256 )
		//{
		//	index = ctrl_code & 0x7F; 
		//	log_trace( ( MSG_INFO, "index param is %d\n", index ) ); 

		//	if( index >= 0 && index < work_proc_count )
		//	{
		//		log_trace( ( MSG_INFO, "restart process %d\n", index ) ); 
		//		end_process( index ); 
		//		start_process( index );
		//	}
		//	else if( index == 127 )
		//	{
		//		log_trace( ( MSG_INFO, "restart all process %d\n", index ) ); 
		...
		//else
		//{
		//	log_trace( ( DBG_MSG_AND_ERROR_OUT, "Unrecognized opcode %d\n", ctrl_code ) ); 
		//}
		break; 
	}
...
BOOL dump_service_info( LPCTSTR szSvcName )
{
	...
	//MessageBox( NULL, lpqscBuf->lpBinaryPathName, NULL, 0 ); 
...
LRESULT get_service_exe_name( LPCSTR szSvcName, LPSTR binary_path, ULONG name_len )
{
	...
	//#define SERVICE_STORE_KEY_PATH "HKEY_LOCAL_MACHINE\\System\\CurrentControlSet\\Services\\"
	////GetServiceKeyName( )
	//
	//_return:
	//	return ret; 
