...
#define URL_RULE_TITLE _T( "url路径或域名规则定义" )
#define URL_NAME_TXT _T( "url路径或域名")
#define FILE_RULE_TITLE _T( "文件规则定义" )
#define FILE_NAME_TXT _T( "文件路径" )
#define REG_RULE_TITLE _T( "注册表规则定义" )
#define REG_NAME_TXT _T("注册表路径")
...
public:
  ...
  //      ASSERT( m_pRestoreBtn != NULL ); 
	...
	void on_apply()
	{
		...
		{
			show_msg( GetHWND(), _T( "路径是必须输入的关键参数" ), NULL, _T( "输入错误" ) ); 
			goto _return; ; 
		}

		if( name_edit->GetText().GetLength() > _MAX_URL_LEN )
		{
			show_msg( GetHWND(), _T( "路径长度不得大于1024" ), NULL, _T( "输入错误" ) ); 
			goto _return; ; 
		}

		if( desc_edit->GetText().GetLength() > _MAX_DESC_LEN )
		{
			show_msg( GetHWND(), _T( "描述长度不得大于512" ), NULL, _T( "输入错误" ) ); 
			goto _return; 
		}

		if( app_name_edit->GetText().GetLength() > _MAX_FILE_NAME_LEN )
		{
			show_msg( GetHWND(), _T( "描述长度不得大于260" ), NULL, _T( "输入错误" ) ); 
			goto _return; 
		}
