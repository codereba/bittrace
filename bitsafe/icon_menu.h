...
class icon_menu : public CWindowWnd, public INotifyUI
{
public:
	icon_menu() : m_pOwner(NULL), can_close( FALSE )
	{
	...
	void Init(CControlUI* pOwner, POINT pt)
	{
		...
		wnd_created = Create( GetDesktopWindow(), NULL, WS_POPUP, WS_EX_TOOLWINDOW | WS_EX_TOPMOST, 100, 100, 300, 300 ); 
		
		if( wnd_created == NULL 
			|| m_hWnd == NULL 
			|| FALSE == IsWindow( wnd_created ) 
		...
		//ctrl = m_pm.FindControl( _T( "menu_layout" ) ); 
		//ASSERT( ctrl != NULL ); 
		//( ( CContainerUI* )ctrl )->SetInset( CRect( 1, 1, 1, 1 ) ); 
		
		//ctrl->SetBkImage( _T( "file='menu_bk.png' corner='30,3,3,3' fade='255' hole='false'" ) ); 
		////ctrl->SetBkColor( 0xffffffff ); 

		//ctrl = m_pm.FindControl( _T( "icon_menu" ) ); 
		//ASSERT( ctrl != NULL ); 
		//( ( CContainerUI* )ctrl )->SetInset( CRect( 30, 8, 8, 8 ) ); 

		//menu_list = m_pm.FindControl( _T( "icon_menu" ) ); 
		//ASSERT( menu_list != NULL ); 
		
		//menu_list->GetItemAt( 0 ); 
...
_return:
	...
	void Notify( TNotifyUI& msg )
	{
		...
		{
			if( msg.pSender->GetName() == _T( "icon_menu" ) )
			{
				if( IsWindow( m_hWnd ) == TRUE )
				{
					HWND tmp_wnd; 
	...
	LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		{
			pRoot = builder.Create(_T("icon_menu.xml"), (UINT)0, NULL, &m_pm);
		}
		//else
		//{
		//	pRoot = builder.Create(_T("icon_menu_en.xml"), (UINT)0, NULL, &m_pm);
		//}
