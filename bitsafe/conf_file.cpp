...
//#define ADD_RULE_DBG_PTR ( PVOID )0xc7000031
//INT32 add_rule( LPCTSTR app_path, RULE_TYPE type, ACTION_TYPE action )
//{
//	INT32 ret = 0; 
//	LPCTSTR tbl_name; 
//	TCHAR sql_stmt[ SQL_STMT_LEN ]; 
...
//	switch( type )
//	{
//	case FW_APP_NAME_RULE:
//	case FW_APP_CLS_NAME_RULE:
//	case FW_URL_NAME_RULE:
//	case FW_URL_CLS_NAME_RULE:
//	case FW_IP_REGION_RULE:
//	case FW_IP_REGION_CLS_RULE:
//		db = ( sqlite3* )action_rule_db; 
//		tbl_name = FW_RULE_TABLE; 
//		break; 
//
//
//	case DEFENSE_APP_NAME_RULE:
//	case DEFENSE_APP_CLS_NAME_RULE:
//	case DEFENSE_COM_NAME_RULE:
//	case DEFENSE_COM_CLS_NAME_RULE:
//	case DEFENSE_FILE_NAME_RULE:
//	case DEFENSE_FILE_CLS_NAME_RULE:
//	case DEFENSE_REG_NAME_RULE:
//	case DEFENSE_REG_CLS_NAME_RULE:
//		db = ( sqlite3* )defense_rule_db; 
...
//	ASSERT( db != NULL ); 
//
//	_sntprintf( sql_stmt, SQL_STMT_LEN, SQL_INSERT_APP_RULE, tbl_name, app_path, action ); 
//
//	ret = sql_exec( ( sqlite3* )db, sql_stmt, NULL ); ;
//	if( ret != SQLITE_OK )
//	{
//		__Trace( _T( "insert new firewall trusted app failed: %d, reason %s\n" ), ret, sqlite3_errmsg16( ( sqlite3* )db ) );
//	}
//
//_return:
//	return ret; 
...
//LRESULT modify_rule_from_db( LPCTSTR class_name, 
//						 ULONG id, 
//						 LPCTSTR class_name2, 
//						 ULONG id2, 
//						 LPCTSTR class_name3, 
...
//	ASSERT( is_valid_access_rule_type( type ) == TRUE ); 
//
//	log_trace( ( MSG_INFO, "rule type is %s \n", get_rule_type_desc( type ) ) ); 
//
...
//	}
//
//	//update tempTable set name='check' where name='1' and column2='' and column3=''
//	sql_stmt = ( TCHAR* )malloc( LOG_SQL_STMT_LEN * sizeof( WCHAR ) ); 
//	if( sql_stmt == NULL )
//	{
//		ret = ERROR_NOT_ENOUGH_MEMORY; ; 
...
//	}
//
//	_tcsncpy( sql_stmt, UPDATE_CMD, LOG_SQL_STMT_LEN ); 
//	_tcsncat( sql_stmt, table_name, LOG_SQL_STMT_LEN - _tcslen( sql_stmt ) ); 
//
//	*condition = _T( '\0' ); 
//
//	add_rule_sel_param( condition, id, class_name, &param_added ); 
//	add_rule_sel_param( condition, id2, class_name2, &param_added ); 
//	add_rule_sel_param( condition, id3, class_name3, &param_added ); 
//	add_rule_sel_param( condition, id4, class_name4, &param_added ); 
//	add_rule_sel_param( condition, id5, class_name5, &param_added ); 
//
//	if( *condition == L'\0' )
//	{
...
//		_tcsncat( sql_stmt, condition, LOG_SQL_STMT_LEN - _tcsncat( sql_stmt ) ); 
//	}
//
//	ret = sql_exec( ( sqlite3* )db, sql_stmt, NULL ); 
//	if( ret != SQLITE_OK )
...
LRESULT open_bitsafe_conf()
{
	...
	if( ret != SQLITE_OK )
	{
		if( ret == SQLITE_NOTADB )
		{
			...
			ret = restore_action_log_db_backup( ACTION_FILTER_COND_DB_NAME ); 
			if( ret != ERROR_SUCCESS )
			{
				log_trace( ( MSG_ERROR, "create access rule db failed \n" ) );
				goto _return; 
...
LRESULT close_bitsafe_conf()
{
	...
	{
		ret = ERROR_DB_CLOSE_ERROR; 
		__Trace( _T( "close configure db error: %d, reason %s\n" ), ret, sqlite3_errmsg( ( sqlite3* )common_log_db ) );
	}
	//cond_db = NULL; 
...
LRESULT sqlite_backup()
{
  ...
  */
  ...
  do
  {
	  ...
	  //"PRAGMA page_size=1024;" "ATTACH '%s' AS aux" "PRAGMA journal_mode=DELETE"
	  pBackup = sqlite3_backup_init(pDest, "aux", pSrc, "temp");
	  if( pBackup==0 ){
		  return 0;
	  }
  ...
  /* Copy into a TEMP database that does not yet exist */
  ...
  */
  ...
  //#endif
  //  sqlite3_backup_init(pDest, "temp", pSrc, "temp");
  //  rc = sqlite3_backup_step(pBackup, 9999);
  //  //rc==SQLITE_DONE;
  //  sqlite3_backup_finish(pBackup);
...
LRESULT add_test_url_rule()
{
	...
	rule_input.desc.params[ 1 ].app.app_name = L"www.codereba.com";  
...
INT32 add_url_rule_from_config( LPCTSTR text, ULONG text_type, ULONG flags, PVOID param )
{
	...
	//ret = init_access_rule( URL_RULE_TYPE, &rule_input ); 
	//if( ret != ERROR_SUCCESS )
	//{
	//	goto _return; 
	//}
...
//	}
//#else
	//filter_text->length = _tcslen( text ) + sizeof( TCHAR ); 
	//filter_text->type = text_type;
	//memcpy( filter_text->url, text, filter_text->length ); 
//#endif //_UNICODE

	//rule_input.desc.params[ 1 ].app.app_name = text;  
...
LRESULT input_rule_from_conf_file( LPCTSTR _file_name )
{
	...
	//ret = add_app_path_to_file_name( file_name, MAX_PATH, _file_name, TRUE ); 
	//if( ret != ERROR_SUCCESS )
	//{
	//	log_trace( ( DBG_MSG_AND_ERROR_OUT, "get the config file name failed\n" ) ); 
	//	goto _return; 
	//}
...
LRESULT input_driver_state( bitsafe_function_type type, bitsafe_function_state load_state )
{
	...
	sqlite3 *db; 
	TCHAR sql_stmt[ DRV_CONF_SQL_STMT_LEN ];
	//sql_stmt = malloc( sizeof( LOG_SQL_STMT_LEN ) * sizeof( WCHAR ) ); 
...
LRESULT config_all_driver()
{
	...
	_ret = config_driver( HTTP_URL_FLT_DRIVER, BITSAFE_FUNCTION_DISABLED ); 
	if( _ret != ERROR_SUCCESS )
	{
		log_trace( ( MSG_INFO, "write driver config error 0x%0.8x\n", _ret ) ); 
		ret = _ret; 
	...
	_ret = config_driver( SYS_MANAGE_FUNCTION, BITSAFE_FUNCTION_DISABLED ); 
	if( _ret != ERROR_SUCCESS )
	{
		log_trace( ( MSG_INFO, "write driver config error 0x%0.8x\n", _ret ) ); 
		ret = _ret; 
	...
	_ret = config_driver( NET_FW_DRIVER, BITSAFE_FUNCTION_DISABLED ); 
	if( _ret != ERROR_SUCCESS )
	{
		log_trace( ( MSG_INFO, "write driver config error 0x%0.8x\n", _ret ) ); 
		ret = _ret; 
	...
	_ret = config_driver( SEVEN_FW_WIN7_DRIVER, BITSAFE_FUNCTION_DISABLED ); 
	if( _ret != ERROR_SUCCESS )
	{
		log_trace( ( MSG_INFO, "write driver config error 0x%0.8x\n", _ret ) ); 
		ret = _ret; 
...
//typedef struct _all_drv_state_input
//{
//	drv_open_state *drv_states; 
//	ULONG max_count; 
//	ULONG cur_count; 
//} all_drv_state_input, *pall_drv_state_input; 
...
LRESULT CALLBACK read_drv_state_callback( bitsafe_function_type type, bitsafe_function_state state, PVOID param )
{
	...
	//if( input->cur_count >= input->max_count )
	//{
	//	ret = ERROR_ERRORS_ENCOUNTERED; 
	//	ASSERT( FALSE && "how much time input to the driver state?" ); 
	//	goto _return; 
	//}
...
LRESULT load_driver_conf( bitsafe_function_state all_drv_state[ MAX_FUNCTION_TYPE ] )
{
	...
	//input.cur_count = 0; 
	//input.max_count = MAX_DRIVER_TYPE; 
	//input.drv_states = all_drv_state; 
	...
	if( ret != ERROR_SUCCESS )
	{
		...
		//for( i = input.cur_count; i < MAX_DRIVER_TYPE; i ++ )
		//{
		//	all_drv_state[ i ].id = MAX_DRIVER_TYPE; 
		//	all_drv_state[ i ].state = DRV_STATE_UNLOADED; 
		//}
	}
...
LRESULT init_bitsafe_function_state()
{
	...
	ASSERT( drv_open_state_lock.DebugInfo->CriticalSection == &drv_open_state_lock ); 
...
LRESULT stop_test_conf_function()
{
	...
	//del_trace_log( TRACE_LOGGER_SYSTEM ); 
	return ret; 
}
...
LRESULT test_socket_rule()
{
	...
	{	
		...
		//rule_input.desc.socket.src_port.port.port_begin = 19; 
		//rule_input.desc.socket.src_port.port.port_end = 19; 

		//rule_input.desc.socket.dest_port.port.port_begin = 19; 
		//rule_input.desc.socket.dest_port.port.port_end = 102; 

		rule_input.resp = ( action_response_type )( i % MAX_RESPONSE_TYPE ); 

		//ret = input_rule_to_config( &rule_input, APP_DEFINE_RULE ); 
		//if( ret != ERROR_SUCCESS )
		//{
		//	log_trace( ( MSG_ERROR, "add the socket action rule failed %ws \n", "0 - 255.255.255.255" ) ); 
		//}

		//ret = config_rule( &rule_input, NULL, 0 ); 
		//if( ret == ERROR_SUCCESS )
		//{
	...
	for( i = 0; i < TEST_REGION_NUM; i ++ )
	{
		...
		//ret = str_2_ipv4_addr( _T( "255.255.255.255" ), &socket_action.socket_info.dest_ip ); 
		//if( ret != ERROR_SUCCESS )
		//{
		//	ASSERT( FALSE ); 
		//	goto _return; 
		...
		ASSERT( FALSE ); 
		break; 
		//socket_action.socket_info.dest_ip = i * region_len + ;  
...
INT32 test_conf_function( ULONG mode )
{
	...
	if( test_add == TRUE )
	{
		for( i = 0; i < 100; i ++ )
		{
			if( mode & TEST_REG_RULE )
			{
				...
				//wcsncpy( _fw_log.proc_name, test_str.GetData(), _MAX_FILE_NAME_LEN ); 

				//_fw_log.id = 0; 
				//_fw_log.type = OTHER_ACTION; 
				//_fw_log.resp = ACTION_BLOCK; 
				//_fw_log.prot = PROT_TCP; 
				//*_fw_log.desc = _T( '\0' ); 
				//_fw_log.dest_port = i; 
				//_fw_log.src_ip = i + 100; 
				//_fw_log.dest_ip = i + 100; 
				//_fw_log.src_port = i; 

				//time( &_fw_log.time ); 

				//ret = add_fw_log( &_fw_log ); 
				//if( ret != SQLITE_OK )
				//{
				//	ASSERT( FALSE ); 
				//	goto _return; 
				...
				//wcscpy( _defense_log.proc_name, test_str.GetData() ); 
				//_defense_log.id = 0; 
				//_defense_log.flag = OTHER_ACTION; 
				//_defense_log.resp = ACTION_BLOCK; 
				//*_defense_log.target_desc = _T( '\0' ); 
				//time( &_defense_log.time ); 

				//ret = add_defense_log( &_defense_log ); 
				//if( ret != SQLITE_OK )
				//{
				//	__Trace( _T( "add defense log failed %d\n" ), ret ); 
				//	ASSERT( FALSE ); 
				//	goto _return; 
				//}

				//__Trace( _T( "add regitry rule, path is %s \n" ), test_str.GetData() ); 

				//ret = add_reg_param_record( param_define_db, test_str.GetData(), _T("test" ), _T( "test2" ), 0 ); 
				//if( ret != SQLITE_OK  )
				//{
				//	__Trace( _T( "add reg parameter failed %d\n" ), ret ); 
				//	ASSERT( FALSE ); 
				//	goto _return; 
				//}

				//ret = add_com_param_record( param_define_db, test_str.GetData(), _T("test" ), _T( "test2" ), 0 ); ; 
				//if( ret != SQLITE_OK )
				//{
				//	__Trace( _T( "add com parameter failed %d\n" ), ret ); 
				//	ASSERT( FALSE ); 
				//	goto _return; 
				//}

				//ret = add_file_param_record( param_define_db, test_str.GetData(), _T("test" ), _T( "test2" ), 0 ); ; 
				//if( ret != SQLITE_OK )
				//{
				//	__Trace( _T( "add file parameter failed %d\n" ), ret ); 
				//	ASSERT( FALSE ); 
				//	goto _return; 
				//}

				//ret = add_ip_param_record( param_define_db, i, 100 + i, _T( "test1" ), _T( "test2"), 0 ); 

				//if( ret != SQLITE_OK )
				//{
				//	__Trace( _T( "add ip parameter failed %d\n" ), ret ); 
				//	ASSERT( FALSE ); 
				//	goto _return; 
				//}

				//ret = add_port_param_record( param_define_db, i, 1000 + i, PROT_TCP, _T( "test1" ), _T( "test2"), 0 ); 

				//if( ret != SQLITE_OK  )
				//{
				//	__Trace( _T( "add port parameter failed %d\n" ), ret ); 
				//	ASSERT( FALSE ); 
				//	goto _return; 
				//}

				//ret = add_url_param_record( param_define_db, test_str.GetData(), _T( "test1" ), _T( "test2"), 0 ); 

				//if( ret != SQLITE_OK )
				//{
				//	__Trace( _T( "add url parameter failed %d\n" ), ret ); 
				//	ASSERT( FALSE ); 
				//	goto _return; 
				//}

				//ret = add_app_param_record( param_define_db, test_str.GetData(), _T( "test1" ), _T( "test2"), 0 ); 

				//if( ret != SQLITE_OK  )
				//{
				//	__Trace( _T( "add app parameter failed %d\n" ), ret ); 
				//	ASSERT( FALSE ); 
				//	goto _return; 
				//}

				//test_str.Format( _T( "test rule %d" ), i ); 

				//rule_input.resp = ( action_response_type )( i % MAX_ACTION ); 

				//*rule_input.addition = L'\0'; 
				//rule_input.type = ( access_rule_type )( i % MAX_ACTION_RULE_TYPE ); 
				...
				//rule_input.desc.params[ 0 ].app.app_name = test_str.GetData(); 

				//rule_input.desc.params[ 1 ].type = APP_DEFINE; 
				//rule_input.desc.params[ 1 ].id = INVALID_ID_VALUE; 
				//rule_input.desc.params[ 1 ].is_cls = FALSE; 

				//rule_input.desc.params[ 1 ].app.app_name = test_str.GetData(); 
	...
	//get_fw_log( check_fw_log, NULL, NULL ); 
	//get_defense_log( check_defense_log, NULL, NULL ); 
	//param_input_desc param_find; 
	//memset( &param_find, 0, sizeof( param_find ) ); 

	//param_find.is_cls = FALSE; 
	//param_find.id = INVALID_ID_VALUE; 
	//param_find.type = FILE_DEFINE; 

	//get_param_define( &param_find, check_file_define, NULL, NULL ); 

	//param_find.type = APP_DEFINE; 
	//get_param_define( &param_find, check_app_define, NULL, NULL ); 

	//param_find.type = REG_DEFINE; 
	//get_param_define( &param_find, check_reg_define, NULL, NULL  ); 

	//__Trace( _T( "get com defines\n" ) ); 
	//param_find.type = COM_DEFINE; 
	//get_param_define( &param_find, check_com_define, NULL, NULL  ); 

	//param_find.type = URL_DEFINE; 
	//__Trace( _T( "get url defines\n" ) ); 
	//get_param_define( &param_find, check_url_define, NULL, NULL  ); 

	//__Trace( _T( "get ip defines\n" ) ); 
	//param_find.type = IP_DEFINE; 
	//get_param_define( &param_find, check_ip_define, NULL, NULL  ); 

	//param_find.type = PORT_DEFINE; 
	//get_param_define( &param_find, check_port_define, NULL, NULL  ); 
	//__Trace( _T( "get fw rules\n" ) ); 

	//get_fw_rule( check_fw_rule, NULL, NULL  ); 

	__Trace( _T( "get defense rules\n" ) ); 
	//get_defense_rule( check_fw_rule, NULL, NULL  ); 
	...
	//	ret = generate_test_action( &action_desc, ( ULONG )i ); 
	//	if( ret != ERROR_SUCCESS )
	//	{
	//		continue; 
	//	}

	//	ntstatus = check_sys_acl( &action_desc, &response, &need_log ); 
	//	if( !NT_SUCCESS( ntstatus ) )
	//	{
	//		ASSERT( response == ACTION_ALLOW ); 

	//		log_trace( ( MSG_INFO, "check the sys action %d error \n", i ) ); 
	//	}
	//	else
	//	{
	//		log_trace( ( MSG_INFO, "check the sys action %d successfully, action is %ws \n", i, get_rule_action_desc( response ) ) ); 
...
LRESULT CALLBACK check_fw_rule( access_rule_type type, action_rule_record *record, PVOID param )
{
	...
	dbg_out.Format( _T( "%s" ), _rule->class_names[ 0 ] ); 
	OutputDebugString( dbg_out.GetData() ); 
	OutputDebugString( _T( "\n" ) ); 

	//*cur_action_desc.addition = L'\0'; 
	//cur_action_desc.id = INVALID_ID_VALUE; 
	//cur_action_desc.resp = record->action; 
	//cur_action_desc.type = INSTALL_DRV; 
	//cur_action_desc.desc.file.app.is_cls = FALSE; 
	//wcscpy( cur_action_desc.desc.file.app.app.app_name, record->class_name1 );  
...
LRESULT add_col_val( sql_stmt_desc *sql_stmt, LPCWSTR col_name, LPCWSTR col_val )
{
	...
	ret = set_col_val( sql_stmt, sql_stmt->col_val_num++, STRING_TYPE, col_name, col_val ); 
...
LRESULT add_col_val_int( sql_stmt_desc *sql_stmt, LPCWSTR col_name, ULONG col_val )
{
	...
	ret = set_col_val( sql_stmt, sql_stmt->col_val_num++, NUMBER_TYPE, col_name, _ultot( col_val, long_val, 10 ) ); 
...
//INT32 is_valid_param( param_all_desc *param )
//{
//	INT32 ret = FALSE; 
//	ULONG param_len; 
//
...
//	}
//
//	//param_len = get_param_define_buf_len( param->type ); 
//	if( must_not_mull_param( param->type ) )
//	{
//		if( param->common.name[ 0 ] == L'\0' )
//		{
//			goto _return; 
//		}
//	}
...
LRESULT del_param( param_all_desc *param, param_define_id *param_id )
{
	...
	//ret = _construct_sql_stmt_by_param( DELETE_ROW, param, &sql_stmt, NULL ); 
	//if( ret != ERROR_SUCCESS )
	//{
	//	goto _return; 
	//}

	//ret = sql_exec( param_define_db, sql_stmt->sql, NULL ); 
	//if( ret != SQLITE_OK )
	//{
	//	if( ret == SQLITE_CONSTRAINT )
	//	{
	//		ASSERT( FALSE && "dont known how to the constraint error happen\n" ); 
	//		ret = SQLITE_OK; 
	//	}
	//	
	//	log_trace( ( MSG_ERROR, "%s delete new record failed: %d, sql %ws extended error code %d reason %ws\n", __FUNCTION__, ret, sql_stmt->sql, sqlite3_extended_errcode( param_define_db ), sqlite3_errmsg16( param_define_db ) ) );
...
//LRESULT del_socket_rule( action_rule_desc *rule )
//{
//	LRESULT ret = ERROR_SUCCESS; 
//
//	ASSERT( rule != NULL ); 
...
//r3_check_sys_action( sys_action_desc *action, event_action_response *resp )
LRESULT r3_check_sys_action( r3_action_notify *action_notify, event_action_response *resp )
{
	LRESULT ret = ERROR_SUCCESS; 
	NTSTATUS ntstatus; 
	...
	access_rule_type type; 
	action_response_type _resp; 
	INT32 need_log = FALSE; 
	data_trace_option _trace_option = { 0 }; 
	BOOLEAN action_checked; 
	BOOLEAN _action_checked; 

	ASSERT( action_notify != NULL ); 
	ASSERT( is_valid_action_type( action_notify->action.action.type ) == TRUE ); 
	ASSERT( resp != NULL ); 

	resp->action = ACTION_ALLOW; 
	resp->id = INVALID_ID_VALUE; 
	resp->need_record = RECORD_NONE; 

	do 
	{
		action_checked = TRUE; 

		action = &action_notify->action; 

		//dump_socket_action( &action->action.socket_info ); 

		ntstatus = check_system_key_proc( action->ctx.proc_name ); 
		if( NT_SUCCESS( ntstatus ) )
		{
			log_trace( ( MSG_INFO, "is system key process\n" ) ); 
			break; 
		}

		ntstatus = find_app_response_record( action->ctx.proc_name, 
			wcslen( action->ctx.proc_name ) + 1, 
			&_trace_option, 
			&_resp ); 
		...
		if( NT_SUCCESS( ntstatus ) )
		{
			...
			log_trace( ( MSG_INFO, "have application rule, response is %ws\n\n", get_action_resp_desc( _resp ) ) ); 
			
			break; 
		}

		ntstatus = find_response_record( action->action.type, 
		...
		if( NT_SUCCESS( ntstatus ) )
		{
			...
			log_trace( ( MSG_INFO, "have action type rule, response is %ws\n\n", get_action_resp_desc( _resp ) ) ); 

			break; 
		}

#ifdef CHECK_DIRECT_FROM_DB
		type = acl_type( action->action.type ); 

		switch( type )
		{
		case URL_RULE_TYPE:
			ret = check_url_rule( action, resp ); 
			break; 

		case SOCKET_RULE_TYPE:
			ret = check_socket_rule( action, resp ); 
			break; 
		case FILE_RULE_TYPE:
			ret = check_file_rule( action, resp ); 
			break; 
		case REG_RULE_TYPE:
			ret = check_reg_rule( action, resp ); 
			break; 
		case COM_RULE_TYPE:
			ret = check_com_rule( action, resp ); 
			break; 
		case COMMON_RULE_TYPE:
			ret = check_common_rule( action, resp ); 
			break; 
		default:
			ret = ERROR_INVALID_PARAMETER; 
			break; 
...
#endif //CHECK_DIRECT_FROM_DB
	}while( FALSE );

	do 
	{
		if( FALSE == action_checked )
		{
			_resp = ACTION_ALLOW; 

			ntstatus = check_sys_action( action_notify, &_resp ); 

			if( ntstatus != STATUS_SUCCESS )
			{
				//log_trace( ( MSG_IMPORTANT ) ); 
...
LRESULT input_r3_rule_from_action( sys_action_desc *sys_action, ULONG flags )
{
	...
	switch( flags )
	{
	case APP_DEFINE_RULE:
		ntstatus = add_app_action_type_rule( rule_input );
		if( !NT_SUCCESS( ntstatus ) )
		{
			log_trace( ( MSG_ERROR, "input the app rule to acl of ring3 error 0x%0.8x\n", ntstatus ) ); 
			ret = ntstatus; 
		}
		break; 
	case APP_ACTION_RULE:
		ntstatus = add_action_rule( rule_input, MODIFY_RULE ); 
		if( !NT_SUCCESS( ntstatus ) )
		{
			log_trace( ( MSG_ERROR, "input the action rule to acl of ring3 error 0x%0.8x\n", ntstatus ) ); 
			ret = ntstatus; 
		}
		break; 
	case APP_ACTION_TYPE_RULE:
		ntstatus = add_app_action_type_rule( rule_input ); 
		if( !NT_SUCCESS( ntstatus ) )
		{
			log_trace( ( MSG_ERROR, "input the action type of the app rule to acl of ring3 error 0x%0.8x\n", ntstatus ) ); 
			ret = ntstatus; 
		}
		break; 
	default:
		ASSERT( FALSE && "why input the unknown mode of action recording" ); 
		goto _return; 
		break; 
	}
...
#define NAME_PARAM_INDEX 1
	...
	//filt_rule = ( PFILTER_URL_INPUT )malloc( sizeof( FILTER_URL_INPUT ) + strlen( url ) + sizeof( CHAR ) ); 
	...
	//filt_rule->length = strlen( url ) + sizeof( CHAR ); 

	//strcpy( filt_rule->url, url ); 
...
LRESULT _set_socket_debug_info( ULONG ip, ULONG port )
{
	...
	//*socket_action_debug.desc.socket.app.app.app_name = L'\0'; 
...
LRESULT debug_sys_action( sys_action_desc *action )
{
	...
	do 
	{
		...
		case SOCKET_CREATE:
		case SOCKET_CONNECT:
			ret = check_socket_debug_info( action->desc.socket.dest_ip.ip.ip_begin, 
				action->desc.socket.dest_port.port.port_begin ); 
			break; 
